...
CourseworkWidget::CourseworkWidget(QWidget *parent) // constructor sets up the project
  : QGLWidget(parent),
    _angle(0),
    _time(0),
  slider_1_angle(0),
...
void CourseworkWidget::initializeGL()
	{ // initializeGL()
...
void CourseworkWidget::rotateAround(int i){
    slider_1_angle = i;
    this->repaint();
}

void CourseworkWidget::moveUpDown(int i){
    slider_2 = i;
    this->repaint();
}

void CourseworkWidget::zoomIn(int i){
    slider_3 = i;
    this->repaint();
}

void CourseworkWidget::panLeftRight(int i){
    slider_4 = i;
    this->repaint();
}

void CourseworkWidget::floor(const materialStruct* p_front){
...
void CourseworkWidget::board(const materialStruct* p_front){
...
void CourseworkWidget::treeBasic(){ //Creates a basic tree without branches.
...
void CourseworkWidget::treeBranches(){ //Creates a tree with branches.
...
void CourseworkWidget::pyramid(const materialStruct* p_front){
...
void CourseworkWidget::sphere(const materialStruct* p_front ){ //sphere from the tutorial, used in one part, otherwise gluSphere is used
...
void CourseworkWidget::cylinder(const materialStruct* p_front){ //used in some parts, provided from the tutorial. Other parts use gluCylinder
...
void CourseworkWidget::windHead(const materialStruct* p_front){ //head of the turbine(where blades connect) is made using sphere provided
...
void CourseworkWidget::windBlade(const materialStruct* p_front){ //blade made using glyCylinder
...
void CourseworkWidget::windTrunk(const materialStruct* p_front){ //using the method provided in the tutorial to generate a cylinder
...
void CourseworkWidget::windMill(double time){
...
void CourseworkWidget::tankHead(const materialStruct* p_front){ //head for the tank, made using gluCylinder
...
void CourseworkWidget::tankHeadCover(const materialStruct* p_front){ // cover for tank head, made using gluDisk
    glMaterialfv(GL_FRONT, GL_AMBIENT,    p_front->ambient);
    glMaterialfv(GL_FRONT, GL_DIFFUSE,    p_front->diffuse);
    glMaterialfv(GL_FRONT, GL_SPECULAR,   p_front->specular);
    glMaterialf(GL_FRONT, GL_SHININESS,   p_front->shininess);
...
void CourseworkWidget::tankCannon(const materialStruct* p_front){ //cannon for tank, made using gluCylinder
...
void CourseworkWidget::cube(const materialStruct* p_front){ //cube used for a few different objects such as tank.
...
void CourseworkWidget::tankWheels(const materialStruct* p_front){ //wheels of the tank, made using gluCylinder
    glMaterialfv(GL_FRONT, GL_AMBIENT,    p_front->ambient);
    glMaterialfv(GL_FRONT, GL_DIFFUSE,    p_front->diffuse);
    glMaterialfv(GL_FRONT, GL_SPECULAR,   p_front->specular);
    glMaterialf(GL_FRONT, GL_SHININESS,   p_front->shininess);
...
void CourseworkWidget::base(){ // base of the tank ( bottom part of the tank hierachy)
...
void CourseworkWidget::barn(){ //barn(house) is made from a cube and a pyramid
    glPushMatrix();
